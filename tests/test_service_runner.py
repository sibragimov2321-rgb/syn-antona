...
def test_railway_service_roles_are_isolated():
    ...
    controlled = command_for_role("controlled_live")
    telegram = command_for_role("telegram")

    assert "app.shadow.disabled" in shadow
    assert "app.trading.controlled_live_runner" in controlled
    assert "app.telegram.runner" not in shadow
    assert "app.telegram.runner" in telegram
    assert len({tuple(shadow), tuple(controlled), tuple(telegram)}) == 3
